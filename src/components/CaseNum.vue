<template>
  <div class="case-num">
    <div class="container">
        <div class="title">
            <span>截止 {{getInfoDate}} 全国数据统计</span>
        </div>
    </div>
    <div class="num">
        <ul class="count">
...
export default {
    ...
    computed:{
        getInfoDate(){
            var date = new Date(this.caseNum.modifyTime);
            var YY = date.getFullYear() + "-";
            var MM = (date.getMonth() + 1) < 10 ? "0" + (date.getMonth() + 1) + "-"  : date.getMonth() + 1 + "-"// 0 - 11
            var DD = date.getDate() < 10 ? "0" + date.getDate() + "   " : date.getDate() + "   "; //1 - 31
